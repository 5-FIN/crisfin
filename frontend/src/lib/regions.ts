/* ────────────────────────────────────────────────
   시도(시/도) 목록 — 복지 데이터의 ctpvNm과 정확히 일치해야 함
──────────────────────────────────────────────── */
...
/* ────────────────────────────────────────────────
   시군구(시/군/구) 목록 — 시도별 하위 행정구역 (2024 기준)
   복지 데이터의 sggNm과 일치해야 함. 세종특별자치시는 단층제라 하위 없음(빈 배열).
──────────────────────────────────────────────── */
export const SIGUNGU_MAP: Record<Sido, readonly string[]> = {
  서울특별시: [
...
/** 주어진 시도의 시군구 목록을 반환(없으면 빈 배열). */
export function sigunguOf(sido: string): readonly string[] {
  return (SIGUNGU_MAP as Record<string, readonly string[]>)[sido] ?? []
}
